...
<div class="success-card">
  ...
  <a href="https://t.me/momentocrypto_bot?start=paid" class="bot-btn">
...
<script>
  ...
  if (window.__mc_send) {
    window.__mc_send('payment_complete', {
      sale: { package: <?= json_encode($package ?? '') ?>, order_id: <?= json_encode($tokens[$token]['order_id'] ?? '') ?>, amount: <?= json_encode(['starter'=>27,'trader'=>72,'pro'=>126][$package] ?? 0) ?> }
    });
  }
