...
$PAYMENT_SYSTEM = $env['PAYOU_SYSTEM_INR'] ?? 'MoneyINR_Sp';
...
if (empty($attribution['landing_url'])) $attribution['landing_url'] = 'https://momentocrypto.com/in';

// INR amounts for Payou UPI payments — India launch pricing
$packages = [
    'starter' => ['amount' => 1400, 'name' => 'Starter — 1 Month'],
    'trader'  => ['amount' => 3800, 'name' => 'Trader — 3 Months'],
    'pro'     => ['amount' => 6700, 'name' => 'Pro — 6 Months'],
];
...
$orderId = $package . '_in_' . time() . '_' . bin2hex(random_bytes(4));
...
// Debug log
file_put_contents(__DIR__ . '/pay_payou_in_debug.log', date('Y-m-d H:i:s') . "\nOrder: {$orderId}\nAmount INR: {$amount}\nSystem: {$PAYMENT_SYSTEM}\nHash: {$hash}\nRedirect: {$redirectUrl}\n\n", FILE_APPEND);
...
if ($tgToken && $tgChat) {
    ...
    $msg = "🟡 UPI payment started (Payou IN)\n\n"
        . "Plan: " . $pkg['name'] . "\n"
        . "Amount: ₹ " . $amount . "\n"
        . "Order: " . $orderId . "\n"
        . "Method: UPI (Payou)\n"
